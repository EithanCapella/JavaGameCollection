...
public class GalagaState extends State {
    ...
    public List<List> grid = initGalagaGrid();
        ...
        if (Mode.equals("Stage")){
        	...
        	if (startCooldown<=0) {
                ...
                if (spawnCooldown == 0 || noEnemies == true) {
                	...
                	if (difficulty.equals("hard") && handler.getScoreManager().getGalagaCurrentScore() >= 5000 && chance <= (score / 2000)) {
                		List<List> grid = getGalagaGrid();
                		...
                		for (int i = 0; i < grid.get(1).size(); i++) {
                			if (grid.get(1).get(i).toString() == "false") {
                				shouldSpawn = true;
                				break;
                			}
                		}
                	...
                	for (int i = 0; i < beesToSpawn; i++) {
                    	...
                    	while (shouldSpawn == true) {
                            ...
                            if (grid.get(row).get(col).toString() == "false") {
                	...
                	for (int i = 0; i < shootersToSpawn; i++) {
                    	...
                    	List<List> shootGrid = grid.subList(1, 3);
                    	...
                    	for (int x = 0; x < shootGrid.size(); x ++) {
                    		for (int y = 0; y < shootGrid.get(x).size(); y ++) {
                    			if (shootGrid.get(x).get(y).toString() == "false") {
                    				shouldSpawn = true;
                    				break;
                    			}
                    		}
                    	...
                    	// Spawn New Enemy
                    	while (shouldSpawn == true) {
                    		int row = handler.getRandomFormation(3, 1);  // random.nextInt(2) + 3;
                            int col = handler.getRandomFormation(7, 1);
    ...
    public void render(Graphics g) {
        ...
        if (Mode.equals("Stage")) {
            ...
            if (champion) {
            	g.drawImage(Images.galagaTrophy, (int) (handler.getWidth() * .05), handler.getHeight()/2, handler.getWidth()/6, handler.getHeight()/4, null);
            }
            
            else if (silver) {
            	g.drawImage(Images.galagaSilverTrophy, (int) (handler.getWidth() * .05), handler.getHeight()/2, handler.getWidth()/6, handler.getHeight()/4, null);
            }
            
            else if (bronze) {
            	g.drawImage(Images.galagaBronzeTrophy, (int) (handler.getWidth() * .05), handler.getHeight()/2, handler.getWidth()/6, handler.getHeight()/4, null);
            }
            ...
            for (int i = 0; i< entityManager.playerShip.getHealth();i++) {
                g.drawImage(Images.galagaPlayer[0], (handler.getWidth() - handler.getWidth() / 4 + handler.getWidth() / 48) + ((entityManager.playerShip.width*2)*i*3/4), handler.getHeight()-handler.getHeight()/4, handler.getWidth() / 18, handler.getHeight() / 18, null);
            }
            if (startCooldown<=0) {
	...
	public List initGalagaGrid() {
    	...
    	for (int i = 1; i < 3; i++) {
			grid.get(i).set(0, "None");
			grid.get(i).set(7, "None");
		}

		return grid;
    }
    
    public List getGalagaGrid() {
    	return grid;
    }
    ...
    private void drawHardModeQuote(Graphics g, int num) {
    	...
    	if (handler.getScoreManager().getGalagaCurrentScore() > 35000 && showChampionPrompt) { // Jann's personal best
        	...
        	g.drawImage(Images.galagaTrophy, (int) (handler.getWidth() * .40), (int) (handler.getHeight() * .60), handler.getWidth()/6, handler.getHeight()/4, null);
        	...
        	g.drawImage(Images.galagaSilverTrophy, (int) (handler.getWidth() * .40), (int) (handler.getHeight() * .60), handler.getWidth()/6, handler.getHeight()/4, null);
        	...
        	g.drawImage(Images.galagaBronzeTrophy, (int) (handler.getWidth() * .40), (int) (handler.getHeight() * .60), handler.getWidth()/6, handler.getHeight()/4, null);
