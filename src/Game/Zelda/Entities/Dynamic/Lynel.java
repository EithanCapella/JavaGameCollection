...
public class Lynel extends BaseMovingEntity{
	...
	public Lynel(int x, int y,BufferedImage[] sprite, Handler handler) {
	...
	public void arrowMethod() {
		...
		if(attacking) {
			if(r.nextInt(25)==1) {
				if(this.sprites==Images.dlynelEnemyFrames) {
					if(animation.getIndex()==0) { //bottom frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.whiteProyectileDown[0], handler,direction.DOWN);
					...
					if(animation.getIndex()==1) { //top frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.whiteProyectileUp[0], handler,direction.UP);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
					if(animation.getIndex()==3) { //right side frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.whiteProyectileSide[0], handler,direction.RIGHT);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
					if(animation.getIndex()==3) { //left side frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.flipHorizontal(Images.whiteProyectileSide[0]), handler,direction.LEFT);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
				}else {
					if(animation.getIndex()==0) { //bottom frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.magicalProyectileDown[0], handler,direction.DOWN);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
					if(animation.getIndex()==1) { //top frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.magicalProyectileUp[0], handler,direction.UP);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
					if(animation.getIndex()==3) { //right side frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.magicalProyectileSide[0], handler,direction.RIGHT);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
					if(animation.getIndex()==3) { //left side frame
						moblinArrow = new moblinArrow(this.x,this.y,Images.flipHorizontal(Images.magicalProyectileSide[0]), handler,direction.LEFT);
						handler.getZeldaGameState().objects.get(handler.getZeldaGameState().mapX).get(handler.getZeldaGameState().mapY).add(moblinArrow);
					}
				}
			}
